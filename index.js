(async()=>{
    ...
    nimyth.clipboard = function(password){
        return new Promise((resolve)=>{
            if(options.accessibility.autoClipboard){
                clipboardy.writeSync(password); nimyth.log("i", "Password has been saved in your clipboard.")
            }else{
    ...
    nimyth.savePassword = function(password, personal){
        return new Promise(async(resolve)=>{
            if(options.accessibility.passwordHistory || personal) nimyth.passwords.self.push({
                password: password,
    ...
    nimyth.askForPassword = function(){
        return new Promise((resolve)=>{
            function ask(){
                ...
                if(!password) return nimyth.log("w", "Please type your password."); setTimeout(ask, 1000 * 3)
    ...
    ================
        ...
        exit                        Exit Nimyth.
            ...
            if(commandArgs[1] === "simple"){
                ...
                password = crypto.randomBytes(+commandArgs[2]).toString("utf8")
            ...
            if(_.find(nimyth.passwords.self, { password: commandArgs[1] })) {
                nimyth.log("i", "Password already added.")
                return nimyth.navigation()
            }
